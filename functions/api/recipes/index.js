...
export async function onRequestPut({ request, env }) {
  ...
  await env.DB.prepare(
    'UPDATE recipes SET title=?, excerpt=?, ingredients=?, instructions=?, category=?, area=?, image=?, youtube=?, author=?, featured=? WHERE id=?'
  ).bind(
    ...
    body.author || 'Готвач БГ', body.featured ? 1 : 0, body.id
  ).run();
